...
const WindPage = () => {
  ...
  // Fetch wind prediction data ONLY when latitude/longitude changes
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // ✅ Calculate dynamic dates (today to 7 days from now)
        ...
        const forecastResponse = await axios.get(
          "https://yantra-ml-wind.onrender.com/predict",
          {
            params: {
              latitude,
              longitude,
              start_date: startDate,       // ✅ Fixed typo & dynamic date
              end_date: endDate,           // ✅ Dynamic date
            },
          }
        );
        ...
        const windData = predictions.map(
          (item: Prediction) => item.Predicted_Power || 0
        );
        ...
        setError(
          error.response?.data?.detail || "Failed to fetch wind data"
        );
      } finally {
        setLoading(false);
      }
